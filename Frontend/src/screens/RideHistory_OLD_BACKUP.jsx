...
function RideHistory() {
  ...
  function classifyAndSortRides(rides) {
    ...
    // Función auxiliar para verificar si una fecha es ayer
    const isYesterday = (date) =>
      date.getFullYear() === yesterday.getFullYear() &&
      date.getMonth() === yesterday.getMonth() &&
      date.getDate() === yesterday.getDate();

    // Función auxiliar para ordenar viajes por fecha (reciente a antiguo)
    const sortByDate = (rides) =>
      rides.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    // Arrays para mantener los viajes clasificados
    const todayRides = [];
  ...
  return (
    <div className="w-full px-3 py-2 border-2 mb-2 rounded-lg cursor-pointer relative">
      <div className="flex flex-wrap gap-2 justify-around">
        ...
        <h1 className="text-sm flex gap-1 items-center font-semibold">
          <CreditCard size={13} className="-mt-[1px] text-black" />COP$ {ride.fare?.toLocaleString('es-CO') || 0}
        </h1>
      </div>
