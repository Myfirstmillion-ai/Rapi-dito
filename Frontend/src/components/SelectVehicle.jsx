...
import { X, ChevronLeft, Check, Clock, Users, ChevronRight } from "lucide-react";
 ...
 * 
 * Features:
 * - AnimatePresence with spring physics (like LocationSearchPanel)
 * - Backdrop with blur effect
 * - Clean white/dark mode adaptive design
 * - Premium vehicle cards with selection ring
...
function SelectVehicle({
  ...
  const [currentlySelected, setCurrentlySelected] = useState(vehicles[0].type);
  ...
  useEffect(() => {
    if (!showPanel) {
      setCurrentlySelected(vehicles[0].type);
    }
  }, [showPanel]);
  ...
  const handleConfirm = () => {
    selectedVehicle(currentlySelected);
    setShowPanel(false);
    showNextPanel(true);
  };
  ...
  const selectedVehicleData = vehicles.find(v => v.type === currentlySelected);
  const currentFare = fare[currentlySelected] || 0;
    ...
    <AnimatePresence>
      {showPanel && (
        <>
          ...
          {/* Bottom Sheet - Matches LocationSearchPanel exactly */}
          <motion.div
          ...
          >
            ...
            <div className="px-5 pb-4 space-y-3">
              {vehicles.map((vehicle) => (
                <motion.div
                  ...
                  onClick={() => setCurrentlySelected(vehicle.type)}
                  className={`flex items-center gap-4 p-4 rounded-2xl cursor-pointer transition-all ${
                    currentlySelected === vehicle.type
            ...
            <div className="px-5 pb-6">
              <motion.button
                ...
                className="w-full h-14 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-lg shadow-lg shadow-emerald-500/25 transition-colors flex items-center justify-center gap-2"
              >
